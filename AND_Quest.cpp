...
pair<bool, vector<int>> bitwise_and_subset(const vector<int> &arr, int K) {
  int n = arr.size();

  // dp[i][bitmask] -> True if bitmask can be achieved using the first i
  // elements
  vector<unordered_map<int, bool>> dp(n);

  // Parent map for reconstructing the subset
  vector<unordered_map<int, int>> parent(n);

  // Initialize the first element
  dp[0][arr[0]] = true;
  parent[0][arr[0]] = -1; // -1 indicates no parent

  // Fill the DP table
  for (int i = 1; i < n; ++i) {
    for (const auto &entry : dp[i - 1]) {
      int bitmask = entry.first;

      // Carry forward the existing subsets
      dp[i][bitmask] = true;
      parent[i][bitmask] = bitmask;

      // Try including arr[i]
      int new_bitmask = bitmask & arr[i];
      if (dp[i].find(new_bitmask) == dp[i].end()) {
        dp[i][new_bitmask] = true;
        parent[i][new_bitmask] = bitmask;
      }
    }

    // Add the current element as a subset itself
    if (dp[i].find(arr[i]) == dp[i].end()) {
      dp[i][arr[i]] = true;
      parent[i][arr[i]] = -1;
    }
  }
  ...
  int bitmask = K;
  for (int i = n - 1; i >= 0; --i) {
    if (parent[i].find(bitmask) != parent[i].end()) {
      if (parent[i][bitmask] != bitmask) { // This means arr[i] was included
        result.push_back(i);
        bitmask = parent[i][bitmask];
      }
    }
  }

  reverse(result.begin(), result.end()); // To get indices in the correct order
  return {true, result};
}
void solve() {
  int n, k;
