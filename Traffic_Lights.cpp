...
void solve()
{
    ...
    while (j--)
    {
        ...
        int diff = abs(*(v.find_by_order(k - 1)) - *(v.find_by_order(k)));
        m.erase(m.lower_bound(diff));
        // bug(a, k, diff);
        m.insert(abs(*(v.find_by_order(k - 1)) - a));
        m.insert(abs(*(v.find_by_order(k)) - a));
        // ṇbug(abs(*(v.find_by_order(k - 1)) - a), abs(*(v.find_by_order(k)) - a));
        v.insert(a);
        cout << *m.begin() << " ";
    }
}
