...
int f(int indx, int prevval)
{
    ...
    int ans = 0;
    auto &x = dp[indx][prevval];
    if (x != -1)
    {
        return x;
    }
    int currloc = loc[indx];
    if (currloc == 0)
    {
        if (v[currloc + 1] == 0)
        {
            for (int i = 1; i <= m; i++)
            {
                ans = (ans + f(indx + 1, i)) % MOD;
            }
        }
        else
        {
            int start = max(1ll, v[1] - 1ll);
            int end = min(m, v[1] + 1);
    ...
    else if (currloc == v.size() - 1)
    {
        int left = v[v.size() - 2] == 0 ? prevval : v[v.size() - 2];
        int start = max((ll)1, left - (int)1);
        int end = min(m, left + 1);
        for (int i = start; i <= end; i++)
        {
            ans = (ans + f(indx + 1, i)) % MOD;
        }
    ...
    {
        int left = v[currloc - 1] == 0 ? prevval : v[currloc - 1];
        int right = v[currloc + 1] == 0 ? -1 : v[currloc + 1];
...
void solve()
{
    ...
    dp.assign(loc.size() + 10, vector<int>(m + 1, -1));
    bool fl = 0;
    for (int i = 0; i < n - 1; i++)
    {
        if (v[i + 1] != 0 and v[i] != 0 and abs(v[i + 1] - v[i]) > 1)
        {
            fl = 1;
            break;
        }
