...
const int frnd = 0;
const int you = 1;
int runs(int i, int prev, int prevprev, v64 &v, int dp[][2][2])
{
    if (i == v.size())
    {
        return 0;
    ...
    int one = 0;
    if (dp[i][prev][prevprev] != -1)
    {
        return dp[i][prev][prevprev];
    }
    if (prev == you and prevprev == you)
    {
        if (v[i] == 1)
        {
            one++;
        }
        return dp[i][prev][prevprev] = (runs(i + 1, frnd, prev, v, dp) + one);
    }
    else if (prev == frnd and prevprev == frnd)
    {
        return dp[i][prev][prevprev] = runs(i + 1, you, prev, v, dp);
    }
    else
    {
        if (v[i] == 1)
        ...
        return dp[i][prev][prevprev] = min(runs(i + 1, frnd, prev, v, dp) + one, runs(i + 1, you, prev, v, dp));
    }
}
void solve()
{
