...
bool pos(int n, int maxJump, int maxSwim, const std::string &terrain)
{
    ...
    while (!nodesQueue.empty())
    {
        ...
        int swimCount = current.second;

        if (index == 0 || terrain[index - 1] == 'L')
        {
            for (int jump = 1; jump <= maxJump; ++jump)
            {
                ...
                if (newIndex == n + 1)
                    return true;
                if (newIndex <= n && terrain[newIndex - 1] != 'C' && !isVisited[newIndex])
                {
                    isVisited[newIndex] = true;
                    nodesQueue.push({newIndex, swimCount});
                }
            }
        }

        if (index > 0 && index <= n && terrain[index - 1] == 'W' && swimCount < maxSwim)
        {
            ...
            {
                isVisited[newIndex] = true;
                nodesQueue.push({newIndex, swimCount + 1});
            }
        }
    }
