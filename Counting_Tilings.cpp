...
int n, m;
void createmask(int i, int mask, vector<int> &ds, int nxt_mask)
{
    if (i == m)
    {
        //   cout << "alert" << endl;
        ds.push_back(nxt_mask);
        return;
    }
    ...
    {
        // cout << "alert1" << endl;
        createmask(i + 1, mask, ds, nxt_mask);
    }
    ...
    if (i < (m - 1))
    {
        ...
        {
            createmask(i + 2, mask, ds, nxt_mask);
        }
    }
    // cout << (mask & (1 << 0)) << endl;
    if ((mask & (1 << i)) == 0)
    {
        // cout << "alert" << endl;
        createmask(i + 1, mask, ds, nxt_mask | (1 << i));
    }
}
int dp[1001][2050];
int f(int r, int mask)
{
    ...
    vector<int> masks;
    createmask(0, mask, masks, 0);
    // printvector(masks);
    int ans = 0;

    for (auto msk : masks)
    {
        ans = (ans % MOD + f(r + 1, msk) % MOD) % MOD;
    }
