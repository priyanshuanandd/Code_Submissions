...
void solve() {
  ...
  int minv = min_element(all(cost)) - cost.begin();
  vector<pair<int, p64>> edges;
  ...
  forn(i, n) {
    if (i != minv) {
      edges.push_back({cost[minv] + cost[i], {i + 1, minv + 1}});
    }
  }
  ...
  int ans = 0;
  for (auto i : edges) {
    if (union_set(i.second.first, i.second.second, parent)) {
      ans += i.first;
    }
  }
