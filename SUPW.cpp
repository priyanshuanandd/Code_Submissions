...
int f(int indx, int prev, vector<int> &v, vector<vector<int>> &dp)
{
    if (indx >= v.size())
    {
        return 0;
    }
    auto &x = dp[indx][prev];// minutes soent till ith index
    ...
    if (prev < 2)
    {
        ans = f(indx + 1, prev + 1, v, dp);
    }

    ans = min(ans, v[indx] + f(indx + 1, 0, v, dp));
