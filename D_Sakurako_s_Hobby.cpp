...
int dfs(int node, vector<int> &adj)
{
    ...
    ans[node] = a;
    // bug(node,ans[node]);
    if (adj[node] == node or vis[adj[node]] == 1)
    {
        return ans[node];
    }
    return ans[node] = a + dfs(adj[node], adj);
}

bool iscycle(int node, vector<int> &vis2, vector<int> &adj)
{
    if (adj[node] == node)
    {
        // cout << "mm" << node << endl;
    ...
    vis2[node] = 1;
    if (iscycle(adj[node], vis2, adj))
    {
        return true;
    }
    return false;
}
void givecycleval(int node, vector<int> &vis2, int val, vector<int> &adj)
{
    ans[node] = val;
    cerr << "aa" << endl;
    if (adj[node] == node or vis2[adj[node]] == 1)
    {
        return;
    }
    vis2[node] = 1;

    givecycleval(adj[node], vis2, val, adj);
}
void solve()
{
    int n;
    ...
    forsn(i, 1, n + 1)
    {
        if (vis[i] == 0)
        {
            // bug(i);
            int count = dfs(i, adj);
            if (iscycle(i, vis2, adj))
            {
                //    bug(i, count);
                givecycleval(i, vis3, count, adj);
            }
        }
        // printvector(vis);
        // printvector(ans);
