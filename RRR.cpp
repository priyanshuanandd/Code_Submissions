...
void solve()
{
    ...
    for (int j = k; j > 0; j--)
    {
        int l = 0;
        int curr = 0;
        for (int r = 0; r <= n - 2 * j; r++)
        {
            if (curr == j)
            {
               // bug(curr);
                if (s[l] == s[r] and s[l] == s[r + curr])
                {
                    ans = max(curr * 3, ans);
                }
                l = r + 1;
            ...
            //bug(l, r, curr, s[r], s[r + j], s[r + 2 * j], j, k);
            if (r != n - 2 * j and s[r] == s[r + j] and s[r + j] == s[r + 2 * j])
            {
                curr++;
            }
            else
            {
                ...
                int f = curr * 3;
                // bug(curr);
                if (r - curr >= 0 and r + curr < n and s[l] == s[r] and s[l] == s[r + curr])
                {
                    ans = max(curr * 3, ans);
                }
                l = r + 1;
