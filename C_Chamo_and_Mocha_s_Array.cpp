...
bool predicate(int target, vector<int> &data)
{
    int min_value = 0, current, previous = 0;
    if (data[0] >= target)
    {
        current = 1;
    }
    ...
    {
        min_value = min(min_value, previous);
        previous = current;
        if (data[i] >= target)
        {
            current++;
