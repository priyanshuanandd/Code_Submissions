...
v64 v;
vector<vv64> dp;
int f(int indx, int positive, int start)
{
    if (indx >= n)
    {
        return 0;
    ...
    int ans = 0;
    auto &x = dp[indx][positive][start];
    if (x != -1)
        return x;
    // pick
    int k = (positive == 1) ? v[indx] : -1 * (v[indx]);
    ans = max(ans, k + f(indx + 1, positive ^ 1, 0));
    // not pick

    ans = max(ans, f(indx + 1, positive, start));
