...
void f(int indx, int sum, vp64 &v, string &s) {
  if (indx == 5) {
    if (sum > 0 and !s.empty()) {
      answer.push_back({sum, s});
    }
    return;
  }

  // pick
  char kk = (char)('A' + v[indx].second);
  string kkk = s + kk;
  f(indx + 1, sum + v[indx].first, v, kkk);
  f(indx + 1, sum, v, s);
}
void solve() {
  int n = 5;
  ...
  sort(all(answer), [&](pair<int, string> &a, pair<int, string> &b) {
    if (a.first == b.first) {
