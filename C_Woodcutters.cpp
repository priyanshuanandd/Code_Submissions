...
int woodcutters(int i, int past, vv64 &dp, v64 &x, v64 &h)
{
    if (i == 0)
    {
        return 0;
    ...
    int ansstay = woodcutters(i - 1, 2, dp, x, h);
    int ansleft = 0;
    if (i == 1 or x[i - 1] - h[i - 1] > x[i - 2])
    {
        ansleft = 1 + woodcutters(i - 1, 1, dp, x, h);
    }
    int ansright = 0;
    if (past == 1)
    {
        ...
        {
            ansright = 1 + woodcutters(i - 1, 0, dp, x, h);
        }
    }
    else if (i == x.size())
    {
    ...
    return dp[i][past] = max({ansstay, ansleft, ansright});
}
