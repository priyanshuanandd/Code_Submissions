...
int f(int indx, int sum, v64 &v, int m)
{
    if (indx == v.size())
    {
        if (sum == 0)
    ...
    int ans1 = f(indx + 1, ((sum - v[indx]) + m) % m, v, m);
    int ans2 = (f(indx + 1, sum, v, m));
   // bug(indx, sum, ans1, ans2);
    return x = ans1 | ans2;
}
// if the sum is divisble by m then all good
void solve()
{
    int n = nxt();
    int m = nxt();
