...
vector<string> unitmaker(int n)
{
    int curr = 3;
    n = pow(3, n);
    vector<string> currv = unit3;
    dotgen(currv.size());
    while (curr != n)
    {
        vector<string> v(curr * 3);
        forn(i, 3)
        {
            forn(i, currv.size())
            {
...
void solve()
{
    ...
    int requnit = pow(3, n - 1);
    vector<string> s = unitmaker(n);
