...
int f(int idx, int rem, string &s, vv64 &dp)
{
    if (idx == s.size())
    {
        return rem == 0;
    ...
    int ans = 0;
    // adding it to curr number
    ans = max(ans, f(idx + 1, (rem + (s[idx] - '0')) % 3, s, dp));
    // breaking sequence
    ans = max(ans, f(idx + 1, (s[idx] - '0') % 3, s, dp) + (rem == 0 ? 1 : 0));
    return x = ans;
}
...
void solve()
{
    ...
    cin >> s;
    // dbg(s);
    vv64 dp(s.size() + 1, v64(3, -1));
    cout << f(0, 0, s, dp) - 1 << endl;
}
