...
bool comp(p64 p1, p64 p2)
{
    if (p1.first == p2.first)
    {
        return p2.second < p1.se;
    }
    return p1 < p2;
}
v64 m;
int runs(int indx, int k, vv64 &dp, v64 &m)
{
    if (k < 0)
    {
        return -2;
    ...
    if (m[indx])
    {
        dp[indx][k] = runs(indx - 1, k - 1, dp, m);
    }
    else
    {
        dp[indx][k] = runs(indx - 1, k, dp, m);
    }

    if (runs(indx - 1, k, dp, m) != -2 and runs(indx - 1, k, dp, m) + m[indx] <= k)
    {
        if (dp[indx][k] == -2 or runs(indx - 1, k, dp, m) + m[indx] <= dp[indx][k])
        {
            dp[indx][k] = runs(indx - 1, k, dp, m) + m[indx];
        }
    }
    return dp[indx][k];
}
void solve()
{
    ...
    //         ans--;
    //     }
    //     // else if (cnt + freq == indx)
    //     // {
    //     //     cnt += freq;
    //     //     ans--;
    //     // }
    // }
    // // ans = min(ans, (int)((m.size() / 2) + m.size() % 2));
    // dbg(ans);

    dp.assign(n + 1, v64(n + 1, -1));
