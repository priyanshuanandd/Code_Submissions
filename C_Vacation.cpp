...
int runs(int indx, int t, vv64 &v, vv64 &dp)
{
    if (indx == 0)
    {
        return 0;
    ...
    if (t == 3)
    {
        return dp[indx][t] = max(max(v[indx - 1][0] + runs(indx - 1, 0, v, dp), v[indx - 1][1] + runs(indx - 1, 1, v, dp)), v[indx - 1][2] + runs(indx - 1, 2, v, dp));
    }
    int ans = 0;
    forn(i, 3)
    {
        if (i != t)
        {
            ans = max(v[indx - 1][i] + runs(indx - 1, i, v, dp), ans);
        }
    }
