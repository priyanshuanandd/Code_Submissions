...
int f(int indx, int cnt)
{
    if (indx == v.size())
    {
        return (cnt > 1) ? 1 : 0; // Valid if non-empty
    }
    auto &x = dp[indx][cnt];
    if (x != -1)
        return x;
    ...
    if (v[indx] % (cnt) == 0)
    {
        ans = (ans + f(indx + 1, cnt + 1)) % MOD;
    }

    ans = (ans + f(indx + 1, cnt)) % MOD;
...
int tabulation(int n)
{
    ...
    // Base case: empty sequence with count > 1 is valid
...
void solve()
{
    ...
    std::cout << tabulation(n) << std::endl;
}
