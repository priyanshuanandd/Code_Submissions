...
void dfs(int x, int y) {
  ...
  forn(i, 4) {
    int nx = x + dx[i];
    int ny = y + dy[i];
    if (nx >= 0 and nx < n and ny >= 0 and ny < m and grid[nx][ny] != '#' and
        vis[nx][ny] != 1) {
      dfs(nx, ny);
    }
  }
...
void bfs(int x, int y, int ex, int ey) {
  ...
  q.push({{x, y}, ""});
  while (!q.empty()) {
    p64 coordi = q.front().first;
    string s = q.front().second;
    q.pop();
    vis[x][y] = 1;
    forn(i, 4) {
      int nx = coordi.first + dx[i];
      int ny = coordi.second + dy[i];
      if (nx >= 0 and nx < n and ny >= 0 and ny < m and grid[nx][ny] != '#' and
          vis[nx][ny] != 1) {
        q.push({{nx, ny}, s + dir[i]});
        if (nx == ex and ny == ey) {
          string ans = s + dir[i];
          cout << ans.size() << ln << ans << endl;
        }
      }
    }
  }
