...
void solve()
{
    ...
    cin >> upperLimit;
    set<int64_t> differences;

    for (size_t index = 1; index < primeList.size(); ++index)
    {
        if (primeList[index] > upperLimit)
        {
            break;
        }
        if (prime[primeList[index] - 2])
        {
            differences.insert(primeList[index] - 2);
        }
        if (prime[primeList[index] - primeList[index - 1]])
        {
            differences.insert(primeList[index] - primeList[index - 1]);
        }
    }
