...
void solve() {
  ...
  p--;
  q--;
  vv64 v(h, vector<int>(w, 0)), vis(h, vector<int>(w, 0));
  for (auto &i : v) {
    cins(i);
  }
  int curr = v[p][q];
  minheap<pair<int, p64>> adj;
  vis[p][q] = 1;
  for (int i = 0; i < 4; i++) {
    int xx = p + dx[i];
    int yy = q + dy[i];
    if (xx >= 0 and xx < h and yy >= 0 and yy < w) {
      adj.push({v[xx][yy], {xx, yy}});
      vis[xx][yy] = 1;
    }
  }
  int a, b;
  ...
  while (!adj.empty()) {
    auto top = adj.top();
    adj.pop();
    int strength = top.first;
    int curr_r = top.second.first;
    int curr_c = top.second.second;

    // Check absorption condition
    if (strength >= (curr + x - 1) / x)
      break;
    ...
    for (int i = 0; i < 4; i++) {
      int xx = curr_r + dx[i];
      int yy = curr_c + dy[i];
      if (xx >= 0 && xx < h && yy >= 0 && yy < w && !vis[xx][yy]) {
        adj.push({v[xx][yy], {xx, yy}});
        vis[xx][yy] = 1;
      }
    }
