...
int f(int indx, int size, vv64 &dp, vp64 &items)
{
    if (indx == items.size() or size == 0)
    {
        return 0;
    }
    ...
    if (x != -1)
        return x;
    int ans = 0;
    if (size >= items[indx].F)
    {
        ans =
            max(ans, items[indx].S +
                         f(indx + 1, size - items[indx].F, dp, items));
    }

    ans = max(ans, f(indx + 1, size, dp, items));
...
void solve()
{
    ...
    cins2(items);
    sort(all(items));
    vv64 dp(n, v64(cap + 1, -1));
    cout << (f(0, cap, dp, items));
}
//! testcase ???????
