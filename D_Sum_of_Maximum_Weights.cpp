...
void solve() {
  int n;
  cin >> n;
  vector<pair<int, p64>> v(n - 1);

  forn(i, n) { cin >> v[i].second.first >> v[i].second.second >> v[i].first; }
  ...
  int ans = 0;
  for (auto i : v) {
    ans += (parents[find_set(i.second.first, parents)] *
            parents[find_set(i.second.second, parents)] * i.first);
    union_set(i.second.first, i.second.second, parents);
    // cout << "PARENTS" << endl;
    // printvector(parents);
    //  bug(i.first);
  }
